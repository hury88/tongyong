...
$(function(){
   ...
   /* 订单评价弹出框*/
    $(".pay-successful-cover").height(payHeight);
    $(".order-evaluate").each(function(){
        console.log("第一步");
        $(this).click(function(){
            $(".pay-successful-cover").fadeIn("fast");
            $(".order-praise-mask").fadeIn("fast").css({
                left: ($(window).width() - $('.order-praise-mask').outerWidth())/2,
                top: ($(window).height() - $('.order-praise-mask').outerHeight())/2
            });
        })
    });

    $(".praise-mask-title a").click(function(){
        $(".pay-successful-cover").fadeOut("fast");
        $(".order-praise-mask").fadeOut("fast");
    });
 ...
 /* 积分提醒弹框*/
    ...
    $(".safety-content-box h3").click(function(){
        $(".pay-successful-cover").fadeIn("fast");
        $(".integral-mask").fadeIn("fast").css({
            left: ($(window).width() - $('.integral-mask').outerWidth())/2,
            top: ($(window).height() - $('.integral-mask').outerHeight())/2
        });
    });
    $(".integral-mask .integral-close").click(function(){
        $(".pay-successful-cover").fadeOut("fast");
        $(".integral-mask").fadeOut("fast");
    });

   /* 简历预览弹框*/
    $(".my-resume-operate .preview").each(function(){
        console.log("第一步");
        $(this).click(function(){
            $(".pay-successful-cover").fadeIn("fast");
            $(".resume-table").fadeIn("fast").css({
                left: ($(window).width() - $('.resume-table').outerWidth())/2,
            });
        })
    });
    $(".resume-cover-close").click(function(){
        $(".pay-successful-cover").fadeOut("fast");
        $(".resume-table").fadeOut("fast");
    });
