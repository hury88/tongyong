...
class Certificate extends Model
{
    ...
    public function v_zhengshulist($where=[],$certificate_lid,$field=['*'],$num=null)
    {
        return $this->parseWhere($where)
            ->where('isstate','=' ,'1')
            ->where('certificate_lid','=' ,$certificate_lid)
            ->latest('isgood')
            ->latest('disorder')
            ->latest('id')
            ->take($num)
    ...
    public function v_zhengshupages($where=[],$certificate_lid,$field=['*'],$num=15,$linknum=5){
        return $this->parseWhere($where)
            ->where("isstate","=" ,"1")
            ->where('certificate_lid','=' ,$certificate_lid)
            ->latest('isgood')
